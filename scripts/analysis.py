...
def generate_datapoints(min_thresh = 0, max_thresh = 1, step_thresh = 0.01):
    ...
    for i in range(num_steps):
        ...
        true_positive = np.sum(np.logical_and(np.uint8(thresholded_score>threshold), label==1))/np.float64(np.sum(label==1))
        false_positive = np.sum(np.logical_and(np.uint8(thresholded_score>threshold), label==0))/np.float64(np.sum(label==0))
        true_negative = np.sum(np.logical_and(np.uint8(thresholded_score<threshold), label==0))/np.float64(np.sum(label==0))
        false_negative = np.sum(np.logical_and(np.uint8(thresholded_score<threshold), label==1))/np.float64(np.sum(label==1))

        pixel_accuracy = np.float64((np.sum(np.logical_and(np.uint8(thresholded_score>threshold), label==1))+np.sum(np.logical_and(np.uint8(thresholded_score<threshold), label==0))))/total_pixels
        pos_pixel_accuracy = (np.sum(np.logical_and(np.uint8(thresholded_score>threshold), label==1)))/total_pixels
        neg_pixel_accuracy = (np.sum(np.logical_and(np.uint8(thresholded_score<threshold), label==0)))/total_pixels
        ...
        IU_positive = np.sum(np.logical_and(np.uint8(thresholded_score>threshold), label==1))/(np.float64(np.sum(label==1))+np.sum(np.logical_and(np.uint8(thresholded_score>threshold), label==0)))
        IU_negative = np.sum(np.logical_and(np.uint8(thresholded_score<threshold), label==0))/(np.float64(np.sum(label==0))+np.sum(np.logical_and(np.uint8(thresholded_score<threshold), label==1)))
        mean_IU = (IU_positive + IU_negative)/2
        frequency_IU = ((np.float64(np.sum(label==1))*IU_positive) +  (np.float64(np.sum(label==0))*IU_negative))/ total_pixels

        datapoints.append([threshold, false_positive, true_positive, false_negative, true_negative, pixel_accuracy, mean_accuracy, mean_IU, frequency_IU])
